...
import com.example.wearthis.data.dto.toDomain
...
class DefaultClothingRepository(
    ...
    override suspend fun refresh(): Result<Unit> = runCatching {
        ...
        remote.forEach { item ->
            val imageUrl = item.imageUrl.toAbsoluteMediaUrl()
            ...
            } else if (existing.remoteImageUrl != imageUrl) {
                clothingDao.update(existing.copy(remoteImageUrl = imageUrl))
            }
        }
    ...
    override suspend fun detail(backendId: String): Result<ClothingDetail> {
        return runCatching { apiService.clothingDetail(backendId).toDomain() }
            .recoverCatching {
                throw it.toApiException("Couldn't load the details for this piece.")
            }
    }

    override suspend fun updateClothing(
        backendId: String,
        request: ClothingUpdateRequestDto
    ): Result<ClothingDetail> {
        return runCatching { apiService.updateClothing(backendId, request).toDomain() }
            .recoverCatching {
                throw it.toApiException("Couldn't update this piece.")
            }
    }
    ...
    override suspend fun addClothing(
        ...
        return runCatching {
            ...
            val entity = ClothingEntity(
                ...
                backendId = null,
                userId = userId,
                localImagePath = withContext(Dispatchers.IO) {
    ...
    override suspend fun retryUpload(localId: String): Result<ClothingItem> {
        ...
        if (entity.userId != sessionStore.userId) {
        ...
        return runCatching {
            ...
            onSuccess = { response ->
                val uploaded = uploading.copy(
                    ...
                    remoteImageUrl = response.imageUrl.toAbsoluteMediaUrl(),
                    uploadStatus = UploadStatus.UPLOADED.name
                )
